...
public class View {
	...
	public View(Board b){
		...
		ArrayList<String> playerList = _board.getPlayerNames();
		_players = new JPanel();
	...
	public void updateTurn(String player){
		...
		for(int i = 0; i < _board.getPlayers().size(); i++){
			...
			panel.add(label1);
			JLabel label2 = new JLabel("Followers: "+_board.getPlayers().ge  //getHash(_board.getPlayers().get(i)));
			panel.add(label2);
	...
	// Array of pixel coordinates of meeple positions on tile image
	 static int[][] meeplePosition = {{23,3}, // 0
									{39,3}, // 1
									{55,3}, // 2
									{73,23}, // 3
									{73,38}, // 4
