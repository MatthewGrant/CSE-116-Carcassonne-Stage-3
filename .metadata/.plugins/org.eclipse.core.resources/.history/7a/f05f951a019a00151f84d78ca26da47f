...
public class Board {
	 ...
	 */
	private HashMap<Player, Integer> _meeple;
	/**
	...
	public Board(ArrayList<Player> players){
		...
		String inside = "river start";
		ImageIcon img = new ImageIcon("resources/25.png"); //new ImageIcon(getClass().getResource("/resources/25.png")
		...
		_tileTypes.createRiverTiles();
		_meeple = new HashMap<Player, Integer>();
		for(int i=0; i<_players.size(); i++) {
	...
	/**
	 * This method takes the Arraylist<Tile> that holds all 11 river tiles used during the game play and shuffles them so that a random tile is pulled each time this method is called. 
	 * 
	 * @return Returns the river tile that is displayed to the user as the tile which can be placed
	 */
	public Tile pickRiverTile(){
		
		//create river tile array list
		ArrayList<Tile> riverTileList = _tileTypes.getRiverTileList();
		...
		//remove last element from array list "river end" tile 
		riverTileList.remove(size-1);
		
		//shuffle list "river Deck" 
		Collections.shuffle(riverTileList);
		
		//add "river end" tile back to the deck
		riverTileList.add(last);
		System.out.println("Size after add " + size);
		// temp tile "tile" is first element in the list 
		Tile tile = riverTileList.get(0);
		
		// removes tile from river deck 
		riverTileList.remove(0);
		return tile;	
	 ...
	 * @return Returns the size of the ArrayList of Tiles from the TileTypes Class as an int
	 */
	public int getRiverTileList(){
		return _tileTypes.getRiverTileList().size();
	}
	...
	/**
	 * This method calls setFollowerSpot() on the tile that was most recently placed, passing in the int that represents
	...
	/**
	 * This is a getter for the ArrayList of players 
	 * 
	 * @return		Returns the ArrayList of players 
	 */
	...
	public Player getPlayer(String name){
		Player player = null;
		for (int i = 0; i < _players.size(); i++ ){
			String playerName = _players.get(i).get_name();
			if( name == playerName){
	...
	public ArrayList<String> getPlayerNames(){
		...
		for (int i = 0; i < _players.size(); i++ ){
			String player = _players.get(i).get_name();
	...
	/**
	 * @return Returns the tile that was just placed
	 */
	public Tile getTile() {
		return _tile;
	}
